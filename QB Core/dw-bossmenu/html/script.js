...
function openHireModal() {    
    ...
    if (currentJobData && currentJobData.grades) {
        ...
        console.error("Aucune classification de poste disponible");
        // Add a default option if no grades are available
        const option = document.createElement('option');
        option.value = "0";
        option.textContent = "Grade par défaut (0)";
        rankSelect.appendChild(option);
    }
...
function updateApplicationsTable() {
    ...
    if (!currentApplications || currentApplications.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="4" class="empty-table">Aucune candidatures à afficher</td></tr>';
        return;
    }
    ...
    if (filteredApplications.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="4" class="empty-table">Aucune candidature correspondante</td></tr>';
        return;
    }
...
function updateApplicationsCount() {
    ...
    countEl.textContent = `${pendingCount} candidatures en attente`;
}
...
function setupChartTimeframeListeners() {
    ...
    timeframeOptions.forEach(option => {
        option.addEventListener('click', function(e) {
            ...
            const buttonText = timeframe === 'day' ? 'Aujourd\'hui' : 
                              timeframe === 'week' ? 'Derniers 7 jours' : 'Derniers 30 jours';
...
function updateEmployeeDropdown() {
    ...
    dropdown.innerHTML = '<option value="">sélectionner un employé</option>';
...
function setupActivityChartControls() {
    ...
    if (expandBtn) {
        expandBtn.addEventListener('click', function() {
            ...
            title.textContent = 'Aperçu des activités - Selon votre heure locale';
...
function updateDashboardStats() {
    ...
    document.getElementById('employees-count').textContent = `Gestion ${currentJobData.employees.length} employés`;
    document.getElementById('employees-management-count').textContent = `Gestion ${currentJobData.employees.length} employés`;
...
function refreshJobData() {
    ...
    .then(jobData => {
        if (jobData) {
            ...
            document.getElementById('department-title').textContent = `${jobData.jobLabel} - Tableau de bord`;
...
window.addEventListener('message', function(event) {
    ...
    if (data.action === 'openUI') {
        ...
        document.getElementById('department-title').textContent = `${data.jobData.jobLabel || data.jobData.jobName || jobName || "Job"} - Tableau de bord`;
...
const formatCurrentQuestion = (currentQuestion) => {
    ...
    switch(currentQuestion.type) {
        ...
        case "select":
            ...
            let optionsHtml = "<option value=''>-- Sélectionnez une option --</option>";
...
const formatButton = (isLast) => {
    isLast ? document.getElementById("finish_task").innerText = "Soumettre" : document.getElementById("finish_task").innerText = "Suivant"
}
